> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [mp.weixin.qq.com](https://mp.weixin.qq.com/s/v4ZzkTQCQa4Li--6wAClKw)
...
但是，要封装一个支持断网重连、自动心跳的 websokect 没有那么容易！
...
我们先看我封装的 websokect，首先，最重要的，它的使用方法和官方 Api 完全一致！零学习成本，上手即用！
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/YBFV3Da0NwvBSia7yN3rn99j2dvQYFT7TenGqJftnf85t7QQNrggzk3YfblB8bWicbvQtBGejn3NRI3ah9QJksiag/640?wx_fmt=other&from=appmsg)

**心跳验证：**

等待一段时间后，我们可以看到 ws 连接里，前端已经发送了多次心跳数据

![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/YBFV3Da0NwvBSia7yN3rn99j2dvQYFT7TEO71S5ibcibWAdcFtgXxog89zZUeN7UFnVBcZxbSozH5YDqT9UY28u0g/640?wx_fmt=other&from=appmsg)

服务端与客户端也一直在进行数据交互

![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/YBFV3Da0NwvBSia7yN3rn99j2dvQYFT7TLJibSh0OCFIEKdttwx5icxuGnPn8vHa5QTnm9Y17mM4TAtzrt8zGL9HQ/640?wx_fmt=other&from=appmsg)

**断网重连验证：**

可以看到，当我们断开服务端的时候，断网重连被自动触发。![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/YBFV3Da0NwvBSia7yN3rn99j2dvQYFT7T0HHdmsO14zT37qYEZicxTicnJk60n3uic09VScuQ1X0Yb3hnicOdesrw8Q/640?wx_fmt=other&from=appmsg)
...
上述代码借助官方 API 实现了一个基本的 WebSocket 客户端，具有以下功能：
...
startHeartbeat() 方法启动一个定时器，每隔 heartbeatInterval 时间（30 秒）发送一次心跳消息。
...
现在，我们已经基本完成了功能的封装，那么，我们如何在外部调用原生的 websokectApi 呢？非常简单，借助几个自定义的生命周期函数即可！
...
注意，这里的 this.dispatchEvent 方法，addEventListener 方法都是通过类继承 EventDispatcher 方法获得的！

EventDispatcher 源码如下：
...
**关于 EventDispatcher 的实现原理，请参考博主的其他文章：**
...
这篇文章封装了 weboskect，完美支持了断网重连、自动心跳的功能，且完全兼容原生写法，无任何学习负担，开开箱即用！但美中不足的是，断网重连时间、心跳数据内容目前都是写死的，大家可以根据自己的情况做一些更改，让它更灵活！
